...
- Server: Postgres con esquema propio dentro de la base compartida del
  ecosistema ATV (DB_PROVIDER=postgres, DB_SCHEMA=roadmaps). Misma convención
  que ecosystem / hiring / clients / ops: una base, un esquema por sistema.
"""
...
DB_SCHEMA = (config("DB_SCHEMA", default="roadmaps") or "roadmaps").strip()


def tabla(nombre: str):
    """Nombre de tabla para Pony. En Postgres va calificado por esquema."""
    return (DB_SCHEMA, nombre) if ES_POSTGRES else nombre


def _bind_sqlite() -> None:
    filename = config("DB_FILENAME", default="data/atv_roadmaps.db")
...
def fetch_all(query) -> list:
    """Materializa un query de Pony sin iterarlo.

    Pony 0.7.19 sobre Python 3.13 rompe al iterar el resultado de `select()`
    (`QueryResultIterator object is not iterable`). Indexar por posición sí
    funciona. Mismo gotcha que en atv-hiring; en Docker (3.12) no aparece, pero
    el código tiene que correr en las dos.
    """
    resultado = query.fetch()
    return [resultado[i] for i in range(len(resultado))]
